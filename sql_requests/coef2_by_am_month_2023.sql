...
WITH
base AS (  -- проекты с платёжом в M-2 и без платежа в M-1
...
den AS (  -- знаменатель: сумма M-2, «сдвинутая» на M
  SELECT
...
num AS (  -- числитель: сумма этих же id в M
  SELECT
    DATE_FORMAT(CAST(CONCAT(b.m2,'-01') AS DATE) + INTERVAL 2 MONTH, '%Y-%m') AS month,
    b.AM,
...
SELECT n.month, n.AM, n.num2, d.den2, n.num2/NULLIF(d.den2,0) AS coef2
FROM num n
